...
基于 OCR 截图识别的胶卷价格追踪工具。📸 粘贴或上传淘宝购物车截图，自动识别商品名称、价格及胶卷属性，保存到数据库并展示趋势图表。
...
- 🖱️ 支持拖拽上传图片
- 📁 支持点击选择文件
- 🔍 OCR 识别后展示商品列表，显示解析标签（品牌、画幅、类型、ISO、有效期）
...
📸 截图 → 🔤 OCR 提取文字 → 🔍 正则匹配价格 → 🏷️ 智能解析胶卷属性 → 💾 保存到数据库
...
|---|---|
| 显式 | `ISO200`, `200度` |
| 隐式 | `金200`, `GOLD200`, `CP200` |
| 型号 | `UltraMax400`, `800T`, `HP5 Plus 400` |
